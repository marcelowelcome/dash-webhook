...
import {
  FIELD_MAP, STATUS_MAP, NUM_COLS, DATE_COLS, BOOL_COLS,
  CONV_MAP, ORC_MAP, DESTINO_NORM,
  parseDate, parseBoolean, parseNumber, coerceFieldValue,
...
// Rolling sync window: 3 hours (cron runs every 2h, so 1h overlap)
const SYNC_WINDOW_MS = 3 * 60 * 60 * 1000

// Elopement pipelines
const ELOPEMENT_PIPELINES = new Set(['Elopment Wedding'])

// Pipeline name → group_id (same as webhook route)
...
async function loadPipelines(headers: Record<string, string>): Promise<Record<string, string>> {
  const resp = await fetchWithRetry(`${AC_API_URL}/api/3/dealGroups?limit=100`, headers)
  if (!resp.ok) return {}
...
async function loadStages(headers: Record<string, string>): Promise<Record<string, string>> {
  const resp = await fetchWithRetry(`${AC_API_URL}/api/3/dealStages?limit=100`, headers)
  if (!resp.ok) return {}
  const data = await resp.json()
  const map: Record<string, string> = {}
...
function buildRecord(
  ...
  // Set group_id from pipeline title if available
  ...
  for (const field of fields) {
    ...
    const val =
      String(field.custom_field_text_value || field.custom_field_text_blob ||
        field.custom_field_date_value || field.custom_field_number_value ||
        field.custom_field_currency_value || field.fieldValue || '').trim()
  ...
  // Fallback: destino, num_convidados, orcamento from marketing form fields
  if (!record.destino) {
...
export async function GET(request: NextRequest) {
  ...
  try {
    // Load pipeline and stage title maps
    const [pipelineMap, stageMap] = await Promise.all([
      loadPipelines(acHeaders),
      loadStages(acHeaders),
    ])

    // Calculate sync window: deals modified in the last 3 hours
    ...
    while (true) {
      pages++
      // AC API v3: filter deals by modification date
      ...
      for (const deal of deals) {
        ...
        // Filter sideloaded fields to this deal
